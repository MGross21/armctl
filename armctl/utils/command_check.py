...
class CommandCheck(Commands):
    ...
    def sleep(seconds: Union[int, float]) -> None:
        ...
        if not isinstance(seconds, (int, float)):
            raise TypeError(f"Sleep duration must be a number, got {type(seconds).__name__}")
        if seconds < 0:
    ...
    def move_joints(robot_instance: Properties, positions: List[float]) -> None:
        ...
        if not isinstance(positions, list):
            raise TypeError(f"Joint positions must be a list, got {type(positions).__name__}")
        ...
        if not all(isinstance(p, (int, float)) for p in positions):
            invalid_types = [type(p).__name__ for p in positions if not isinstance(p, (int, float))]
            raise TypeError(f"All joint positions must be numbers, found: {set(invalid_types)}")
        ...
        for i, (position, (min_limit, max_limit)) in enumerate(zip(positions, joint_ranges)):
    ...
    def move_cartesian(robot_instance: Properties, pose: List[float]) -> None:
        ...
        if not all(isinstance(p, (int, float)) for p in pose):
            invalid_types = [type(p).__name__ for p in pose if not isinstance(p, (int, float))]
            raise TypeError(f"All pose values must be numbers, found: {set(invalid_types)}")
